...
@Autonomous(name = "Auto: RED 4", group = "Autos")   // How opmode is displayed on phones
public class AutoRed4 extends LinearOpMode
{
    ...
    {
        {
            ...
            armLimit = mhw.arm.getCurrentPosition();
        ...
        /** [DRIVING CODE START] */
        // Drive backwards to base plate
        doLinearMovementSensor("backwards", 0.05f, "distRear", 25, false);
        doLinearMovementTime("stop", 0, 0.5);
        // TODO: Lift arm
        doArmMovement(true, 0.4, 1);
        // Turn to face base plate
        doTurn(false, 240, 0.022f);
        doLinearMovementTime("stop", 0, 0.15);
        //put claw down
        doLinearMovementTime("backwards",1,0.15);
        doLinearMovementTime("stop",0,0.15);
        doLinearMovementTime("forwards",1,0.31);
        doLinearMovementTime("stop",0,0.15);
        // TODO: Lower arm
        doArmMovement(false, 0.4, 1.35);
        // Move back to grip plate
        doLinearMovementTime("backwards",0.05f, 0.05);
        doLinearMovementTime("stop",0,0.15);
        // Move to left to make room for turn
        doLinearMovementTime("right", 0.05f, 0.05);
        doLinearMovementTime("stop", 0, 0.15);
        // TODO: Lower arm to grip firmer
        doArmMovement(false,0.1,0.05);
        doLinearMovementTime("backwards",0.1f, 0.05);
        doLinearMovementTime("stop",0,0.15);
        // Turn so base plate is in proper orientation
        doTurn(true, 120, 0.04f);
        doLinearMovementTime("stop", 0, 0.15);
        // Move towards depot
        doLinearMovementTime("forwards", 0.075f, 2);
        doLinearMovementTime("stop", 0, 0.15);
        //doLinearMovementTime("right", 0.075f, 2);
        //doLinearMovementTime("stop", 0, 0.15);
        // TODO: Raise arm
        doArmMovement(true, 0.4, 1);
        // Turn so sensor is to wall
        doLinearMovementTime("back",0.05f,0.05);
        doLinearMovementTime("stop",0,0.15);

        // TODO: Lower arm
        doArmMovement(false, 0.4, 1);
        // Move away from wall a sufficient distance
        doLinearMovementSensor("right", 0.05f, "distTop", 30, false);
        doLinearMovementTime("stop", 0, 0.15);
        // Move sideways until under bridge
    ...
    public static void doArmMovement(boolean up, double power, double seconds) {
        if(up)
            power *= -1;
        else
            power *= 0.35;
        ElapsedTime timer = new ElapsedTime();
        while (timer.time() < seconds && mhw.arm.getCurrentPosition() <= armLimit) {
            mhw.arm.setPower(power);
        }
        mhw.arm.setPower(0);
    }
